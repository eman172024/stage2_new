...
namespace MMSystem.Services.Reports
{
    public class MockReports : IRportInterface
    {
        ...
        public async Task<UserReports> GetAllUserMassageReport(int id, DateTime? fromdate, DateTime? todate, string? MailType, string SendedOrRecieved)
        {
            try
            {
                Administrator user = _data.Administrator.FirstOrDefault(x => x.state == true && x.UserId == id);
                ...
                {                  
                        ...
                        //userreport.Total_Count.TotalOfMassage = userreport.information1.Count();
                        //userreport.Total_Count.TotalOfReplay = massageReplaied;
                        //userreport.Total_Count.TotalOfNotReplay = userreport.Total_Count.TotalOfMassage - userreport.Total_Count.TotalOfReplay;
        ...
        public async Task<UsersConclsionReport> GetAllUserCount(int departmentid, DateTime? fromdate, DateTime? todate, string? MailType, string RecievedOrSended)
        {
            try
            {
              ...
              List<Administrator> user = _data.Administrator.Where(x => x.state == true && x.DepartmentId == departmentid).ToList();
                ...
                else if(RecievedOrSended == "sended")
                {
