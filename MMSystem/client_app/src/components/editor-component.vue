    ...
    <div class="flex justify-center text-center mt-12">
        <div class="flex flex-col">
          <div class="flex justify-between">
            ...
            <button type="button" @click="passToggleShowModal()"
              ...
              <div class="rounded w-10 h-10" :style="{ backgroundColor: color }"></div>

              <Tool 
              :event="() => setTool('eraser')" 
              :iconClass="'fas fa-eraser fa-lg'" 
              :class="{ 'cursor-pointer text-yellow-600': currentActiveMethod === 'eraser' }"
              /> 

              <Tool
              :event="() => setTool('freeDrawing')"
              :iconClass="'fas fa-pencil-alt fa-lg'"
              :class="{ 'cursor-pointer text-yellow-600': currentActiveMethod === 'freeDrawing' }"
              />

              <Tool
              :event="() => setTool('text')"
              :iconClass="'fas fa-font fa-lg'"
              :class="{ 'cursor-pointer text-yellow-600': currentActiveMethod === 'text' }"
              />
              <Tool
              :event="() => setTool('circle')"
              :iconClass="'far fa-circle fa-lg'"
              :class="{ 'cursor-pointer text-yellow-600': currentActiveMethod === 'circle' }"
              />
              ...
              v-show="croppedImage"
              :class="{ 'cursor-pointer text-yellow-600': currentActiveMethod === 'crop' }"
              />

              


              <div class="absolute top-4 left-10 text-white  transform cursor-pointer  rounded-full flex justify-center items-center">
                <button @click="saveImage()" class="top-10 left-10 px-6 py-2 bg-blue-400 rounded-md text-white">
