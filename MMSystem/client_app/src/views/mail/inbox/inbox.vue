            ...
            <div class="flex-1 bg-gray-200 w-0 overflow-y-auto">
                <div class="max-w-screen-2xl  mx-auto flex flex-col md:px-8">
                    ...
                    <main class="flex-1 relative focus:outline-none pt-2 pb-6">
                        <div class="flex justify-between items-center">
                            ...
                            <div class=" flex items-center">
                                ...
                                <span class="flex items-center mr-4">
                                    من 
                                    <input type="date" min="2000-01-01" max="2040-12-30" id="date_from" v-model="date_from" class="block mr-2 w-full rounded-md h-10 border border-gray-200 hover:shadow-sm focus:outline-none focus:border-gray-300 px-2">
                                </span>

                                <span class="flex items-center mr-4">
                                    إلي
                                    <input type="date" min="2000-01-01" max="2040-12-30" id="date_to" v-model="date_to" class="block mr-2 w-full rounded-md h-10 border border-gray-200 hover:shadow-sm focus:outline-none focus:border-gray-300 px-2">
                                </span>
                            </div>
                        ...
                        <div class="w-full mt-4 rounded-md flex items-start divide-y-2 divide-gray-200">
                            <div class="w-7/12 ml-2 "> 

                            البريد
                                        
                                <div class="flex items-center bg-gray-100 w-full text-xs pl-2 py-1 mt-2">
                                    <div class="w-11/12 flex items-center">
                                        <div class="w-2/12 py-1 pr-4">
                                            رقم الرسالة
                                        </div>
                                        <div class="w-3/12">
                                            الحالة
                                ...
                                <div class="min-h-64 text-sm bg-gray-100">
                                    <div v-for="mail in inboxMails" :key="mail.mail_id" :class="mail.flag | mail_state_inbox"  class="group relative border-r-8 border-red-500 flex items-center bg-white hover:bg-gray-100  pl-2">
                                        ...
                                        <button class="w-11/12 flex items-center text-right" @click="to_pass_data_to_get_mail_by_id(mail.mail_id, my_department_id, mail.type_of_mail, mail.sends_id, mail.mangment_sender)">
                                            <div class="w-2/12 pr-4 py-1">
                                                {{ mail.mail_Number }}
                                            </div>
                                        ...
                                        <div class="w-1/12 flex justify-between items-center">
                                            <div class="w-1/3 flex justify-center items-center">
                                                <router-link title="عرض التفصيل" :to="{ name: 'inbox-show', params: { mail: mail.mail_id, department:my_department_id, type:mail.type_of_mail, sends_id:mail.sends_id },}"  class="">
                                                    <svg class="w-4 h-4 fill-current hover:text-green-500" version="1.1" id="Capa_1" x="0px" y="0px" viewBox="0 0 18.453 18.453"  xml:space="preserve">
...
export default {
  ...
  methods: {
    ...
    to_pass_data_to_get_mail_by_id(mailId_to_get_mail_by_id, my_department_id_to_get_mail_by_id, to_test_passing_mail_type_to_get_mail_by_id ,sends_id_to_get_mail_by_id, mangment_sender_to_get_mail_by_id){
