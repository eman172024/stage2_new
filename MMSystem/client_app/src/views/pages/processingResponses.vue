<template>
  <div class="antialiased h-screen flex overflow-hidden bg-gray-100">
    ...
    <div class="flex-1 overflow-auto focus:outline-none" tabindex="0">
      ...
      <main class="max-w-4xl mx-auto mt-8 pb-8 px-4">
              ...
              <div class="sm:col-span-6">
                ...
                <div class="mt-1">
                  <input
                    ...
                    type="text"
                    id="title"
                    class="px-4 shadow-sm focus:ring-cyan-700 focus:border-cyan-700 block mt-1 h-12 w-full sm:text-sm border-gray-300 rounded-md"
                  />
                </div>
              </div>
          ...
          <div class="flex flex-col mt-16">
            <div class="shadow overflow-hidden border-b border-gray-200 sm:rounded-lg">
              <div class="bg-gray-50 flex items-center">
                ...
                <div
                  scope="col"
                  class="w-8/12 px-6 py-3 text-right text-xs font-medium text-gray-500 uppercase tracking-wider"
                >
                  المحتوي
                </div>

                <div scope="col" class="w-31/12 relative px-6 py-3">
              ...
              <div v-for="(response, index) in ProcessingResponses" :key="index" class="flex items-center border-b border-gray-300">
                <div class="w-3/12 px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">
                  {{ response.title }}
                </div>
                <div class="w-8/12 px-6 py-4 whitespace-nowrap text-sm text-gray-500 truncate">
                  {{ response.replyContent }}
                </div>

                <div class="w-1/12 px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                  <button @click="DeleteProcessingResponses(response.replyId, index)" name="btn_delete_response" value="2" type="submit">
                    <svg
                      class="w-4 h-4"
                      height="427pt"
...
export default {
  ...
  methods: {
      ...
      this.$http.processingResponsesService
        ...
        .then((res) => {
          setTimeout(() => {
            ...
            this.ProcessingResponses.unshift({
                replyId: res.data.result.addedrepliy.replyId,
                replyContent: res.data.result.addedrepliy.replyContent,
                title: res.data.result.addedrepliy.title})
                }, 500);
        })
        .catch((err) => {
          setTimeout(() => {
    ...
    DeleteProcessingResponses(id, index){
      this.$http.processingResponsesService
        .DeleteResponsesService(Number(id))
        .then((res) => {
          this.ProcessingResponses.splice(index, 1);
          console.log(res)
